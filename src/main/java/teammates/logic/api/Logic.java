...
public class Logic {
    ...
    protected static final FeedbackResponseCommentsLogic feedbackResponseCommentsLogic = FeedbackResponseCommentsLogic
            .inst();
    ...
    /**
     * Creates a new Account based on given values. If a profile is not given, a
     * default empty profile is created for the user<br>
     * Preconditions: <br>
    ...
    public void createAccount(String googleId, String name, boolean isInstructor, String email, String institute,
        ...
        AccountAttributes accountToAdd = AccountAttributes.builder().withGoogleId(googleId).withName(name)
                .withEmail(email).withInstitute(institute).withIsInstructor(isInstructor)
                .withStudentProfileAttributes(studentProfile).build();
    ...
    /**
     * Preconditions: <br>
     * * All parameters are non-null. This is just for legacy code that creates an
     * Account without the profile parameter
     */
    public void createAccount(String googleId, String name, boolean isInstructor, String email, String institute)
     ...
     * * All parameters are non-null.
     *
     * @return Details of accounts with instruction privileges. Returns empty list
    ...
    /**
     * Deletes both instructor and student privileges. Does not delete courses. Can
     * result in orphan courses (to be rectified in future). Fails silently if no
     * such account. <br>
     * Preconditions: <br>
     * * All parameters are non-null.
     */
    ...
    /**
     * Creates an instructor and an new account if the instructor doesn't not have
     * account yet.<br>
     * Used as a shorthand when the account entity is not important and is only
     * needed for completeness<br>
     * <b>Note: Now used for the purpose of testing only.</b><br>
     * Preconditions: <br>
     * * All parameters are non-null.
     */
    @Deprecated
    public void createInstructorAccount(String googleId, String courseId, String name, String email, Boolean isArchived,
            String roleParam, boolean isDisplayedToStudents, String displayedNameParam, String privileges,
            String institute) throws EntityAlreadyExistsException, InvalidParametersException {
        ...
        if (accountsLogic.getAccount(googleId) == null) {
            AccountAttributes account = AccountAttributes.builder().withGoogleId(googleId).withName(name)
                    .withEmail(email).withInstitute(institute).withIsInstructor(true)
                    .withDefaultStudentProfileAttributes(googleId).build();
            accountsLogic.createAccount(account);
        }

        // In case when roleParam is null, default values used both for role and for
        // privileges.
        // If privileges is null and roleParam is not null, for privileges will be
        // created value based on roleParam
        InstructorAttributes instructor = InstructorAttributes.builder(googleId, courseId, name, email)
                .withRole(roleParam).withDisplayedName(displayedNameParam).withPrivileges(privileges)
                .withIsDisplayedToStudents(isDisplayedToStudents).withIsArchived(isArchived).build();
    ...
    public void addInstructor(String courseId, String name, String email, String role)
        ...
        InstructorAttributes instructor = InstructorAttributes.builder(null, courseId, name, email).withRole(role)
                .withPrivileges(new InstructorPrivileges(role)).build();
        instructorsLogic.createInstructor(instructor);
    }
     ...
     * restrict the visibility according to the logged-in user's google ID. This is
     * used by admin to search instructors in the whole system.
     * 
    ...
    /**
     * Get the encrypted registration key for the instructor. Preconditions: <br>
     * * All parameters are non-null.
     */
    public String getEncryptedKeyForInstructor(String courseId, String email) throws EntityDoesNotExistException {
    ...
    /**
     * Update the name and email address of an instructor with the specific Google
     * ID. Preconditions: <br>
     * * All parameters are non-null.
     * 
     * @param instr
     *            InstructorAttributes object containing the details to be updated
     */
    ...
    public void verifyInputForAdminHomePage(String name, String institute, String email)
        ...
        List<String> invalidityInfo = instructorsLogic.getInvalidityInfoForNewInstructorData(name, institute, email);
    ...
    /**
     * Removes instructor access but does not delete the account. The account will
     * continue to have student access. <br>
    ...
    /**
     * Fails silently if no match found. Preconditions: <br>
     * * All parameters are non-null.
     */
    ...
    public void createCourseAndInstructor(String instructorGoogleId, String courseId, String courseName,
            String courseTimeZone) throws EntityAlreadyExistsException, InvalidParametersException {
    ...
    /**
     * Returns a course data, including its feedback sessions, according to the
     * instructor passed in.<br>
     * Preconditions: <br>
     * * All parameters are non-null.
     */
     ...
     * * All parameters are non-null.
     * 
     * @return A less detailed version of courses for this instructor. Returns an
     ...
     * * All parameters are non-null.
     * 
     * @return A less detailed version of courses for the specified instructor
     *         attributes. Returns an empty list if none found.
     */
    public Map<String, CourseDetailsBundle> getCourseSummariesForInstructors(
     ...
     * * All parameters are non-null.
     *
     * @return Details of courses the student is in. CourseData objects returned
     *         contain details of feedback sessions too (except the ones still
     *         AWAITING).
     */
     ...
     */
    public void updateCourse(CourseAttributes course) throws InvalidParametersException, EntityDoesNotExistException {
     ...
     * 
     * @param courseId
     *            The course of which the archive status is to be changed
     * @param archiveStatus
     *            The archive status to be set
     */
    ...
    /**
     * Deletes the course and all data related to the course (instructors, students,
     * feedback sessions). Fails silently if no such account. <br>
     * Preconditions: <br>
     * * All parameters are non-null.
     */
     ...
     * 
     * @param instructors
     *            a list of InstructorAttributes associated to a googleId, used for
     *            filtering of search result
     * @return Null if no match found
     */
     ...
     * This method should be used by admin only since the searching does not
     * restrict the visibility according to the logged-in user's google ID. This is
     * used by admin to search students in the whole system.
     * 
     * @return Null if no match found.
     */
    ...
    /**
     * Returns a list of {@link SectionDetailsBundle} for a given course using
     * courseId.
     *
     * <p>
     * Preconditions: <br>
    ...
    /**
     * All attributes except courseId be changed. Trying to change courseId will be
     * treated as trying to edit a student in a different course.<br>
     ...
     * Team changed-> creates new submissions for the new team, deletes submissions
     * for previous team structure. <br>
     * Preconditions: <br>
     * * All parameters are non-null.
     */
    ...
    /**
     * Enrolls new students in the course or modifies existing students. But it will
     * not delete any students. It will not edit email address either. If an
     * existing student was enrolled with a different email address, that student
     * will be treated as a new student.<br>
     ...
     *         enrollment. It also includes data for other students in the course
     *         that were not touched by the operation.
     */
    public CourseEnrollmentResult enrollStudents(String enrollLines, String courseId) throws EnrollException,
    ...
    /**
     * Deletes the student from the course including any submissions to/from for
     * this student in this course. Fails silently if no match found. <br>
     * Preconditions: <br>
     * * All parameters are non-null.
     */
    ...
    /**
     * Checks if a course has sections for each team Preconditions: <br>
     * * All parameters are non-null.
     */
     ...
     * * All parameters are non-null.
     */
    public FeedbackSessionAttributes copyFeedbackSession(String copiedFeedbackSessionName, String copiedCourseId,
        ...
        return feedbackSessionsLogic.copyFeedbackSession(copiedFeedbackSessionName, copiedCourseId, feedbackSessionName,
     ...
     * * All parameters are non-null.
     *
     * @see FeedbackSessionsLogic#getFeedbackSessionsListForInstructor(String,
     *      boolean)
     */
     ...
     * * All parameters are non-null.
     *
     * @see FeedbackSessionsLogic#getFeedbackSessionQuestionsForInstructor(String,
     *      String, String)
     */
    public FeedbackSessionQuestionsBundle getFeedbackSessionQuestionsBundleForInstructor(String feedbackSessionName,
        ...
        return feedbackSessionsLogic.getFeedbackSessionQuestionsForInstructor(feedbackSessionName, courseId, null,
                userEmail);
    }

    public FeedbackSessionQuestionsBundle getFeedbackSessionQuestionsBundleForInstructor(String feedbackSessionName,
            String courseId, String questionId, String userEmail) throws EntityDoesNotExistException {

        Assumption.assertNotNull(feedbackSessionName);
        Assumption.assertNotNull(courseId);
        Assumption.assertNotNull(userEmail);

        return feedbackSessionsLogic.getFeedbackSessionQuestionsForInstructor(feedbackSessionName, courseId, questionId,
                userEmail);
    }
     ...
     * * All parameters are non-null.
     *
     * @see FeedbackSessionsLogic#getFeedbackSessionQuestionsForStudent(String,
     *      String, String)
     */
    ...
    public FeedbackSessionQuestionsBundle getFeedbackSessionQuestionsBundleForStudent(String feedbackSessionName,
        ...
        return feedbackSessionsLogic.getFeedbackSessionQuestionsForStudent(feedbackSessionName, courseId, questionId,
                userEmail);
    }

    public FeedbackQuestionAttributes getFeedbackQuestion(String feedbackSessionName, String courseId,
            int questionNumber) {
        Assumption.assertNotNull(feedbackSessionName);
        Assumption.assertNotNull(courseId);
     ...
     * 
     * @see FeedbackSessionsLogic#getFeedbackSessionResultsSummaryAsCsv(String,
     *      String, String, String, boolean, boolean)
     */
    public String getFeedbackSessionResultSummaryAsCsv(String courseId, String feedbackSessionName,
            String instructorEmail, boolean isMissingResponsesShown, boolean isStatsShown, String questionId)
            throws EntityDoesNotExistException, ExceedingRangeException {

        Assumption.assertNotNull(courseId);
        Assumption.assertNotNull(feedbackSessionName);

        return feedbackSessionsLogic.getFeedbackSessionResultsSummaryAsCsv(feedbackSessionName, courseId,
                instructorEmail, questionId, isMissingResponsesShown, isStatsShown);
    }

    /**
     * Generates summary results (without comments) within a section in CSV format.
     * <br>
     * Preconditions: <br>
     * * All parameters(except questionId) are non-null. <br>
     * 
     * @see FeedbackSessionsLogic#getFeedbackSessionResultsSummaryInSectionAsCsv(String,
     *      String, String, String, String, boolean, boolean)
     */
    public String getFeedbackSessionResultSummaryInSectionAsCsv(String courseId, String feedbackSessionName,
            String instructorEmail, String section, String questionId, boolean isMissingResponsesShown,
        ...
        return feedbackSessionsLogic.getFeedbackSessionResultsSummaryInSectionAsCsv(feedbackSessionName, courseId,
                instructorEmail, section, questionId, isMissingResponsesShown, isStatsShown);
    }
    ...
    public void updateRespondents(String feedbackSessionName, String courseId)
            throws InvalidParametersException, EntityDoesNotExistException {
        Assumption.assertNotNull(feedbackSessionName);
        Assumption.assertNotNull(courseId);
    ...
    /**
     * Deletes the feedback session but not the questions and responses associated
     * to it. Fails silently if no such feedback session. <br>
     * Preconditions: <br>
     * * All parameters are non-null.
     */
    ...
    /**
     * Used for creating initial questions for template sessions only. Does not
     * check if feedback session exists. Does not check if question number supplied
     * is valid(does not check for clashes, or make adjustments) Preconditions: <br>
     * * All parameters are non-null. * questionNumber is > 0
     */
    public FeedbackQuestionAttributes createFeedbackQuestionForTemplate(FeedbackQuestionAttributes feedbackQuestion,
            int questionNumber) throws InvalidParametersException {
    ...
    public FeedbackQuestionAttributes copyFeedbackQuestion(String feedbackQuestionId, String feedbackSessionName,
            String courseId, String instructorEmail) throws InvalidParametersException {
     ...
     * The FeedbackQuestionAttributes should have the updated attributes together
     * with the original ID of the question. Preserves null attributes.
     * Preconditions: <br>
     * * All parameters are non-null.
     */
    ...
    /**
     * Returns true if there is at least one response for the given feedback
     * question, false if not. for the session. Preconditions: <br>
     * * All parameters are non-null.
     */
     ...
     * Returns an empty list if they are no questions for the session.
     * Preconditions: <br>
     * * All parameters are non-null.
     */
    ...
    /**
     * Gets a question+response bundle for questions with responses that is visible
     * to the student for a feedback session. Preconditions: <br>
     * * All parameters are non-null.
     */
    ...
    /**
     * Gets a question+response bundle for questions with responses that is visible
     * to the instructor for a feedback session within the given range
     * Preconditions: <br>
     * * All parameters are non-null.
     */
    public FeedbackSessionResultsBundle getFeedbackSessionResultsForInstructorWithinRangeFromView(
        ...
        return feedbackSessionsLogic.getFeedbackSessionResultsForInstructorWithinRangeFromView(feedbackSessionName,
                courseId, userEmail, range, viewType);
    }

    /**
     * Gets a question+response bundle for questions with responses that is visible
     * to the instructor for a feedback session in a section within the given range
     * Preconditions: <br>
     * * All parameters are non-null.
     */
    ...
    /**
     * Gets a question+response bundle for questions with responses that is visible
     * to the instructor for a feedback session from a section within the given
     * range Preconditions: <br>
     * * All parameters are non-null.
     */
    public FeedbackSessionResultsBundle getFeedbackSessionResultsForInstructorFromSectionWithinRange(
        ...
        return feedbackSessionsLogic.getFeedbackSessionResultsForInstructorFromSectionWithinRange(feedbackSessionName,
                courseId, userEmail, section, range);
    }

    /**
     * Gets a question+response bundle for questions with responses that is visible
     * to the instructor for a feedback session to a section within the given range
     * Preconditions: <br>
     * * All parameters are non-null.
     */
    public FeedbackSessionResultsBundle getFeedbackSessionResultsForInstructorToSectionWithinRange(
        ...
        return feedbackSessionsLogic.getFeedbackSessionResultsForInstructorToSectionWithinRange(feedbackSessionName,
                courseId, userEmail, section, range);
    }

    /**
     * Gets a question+response bundle for questions with responses that is visible
     * to the instructor for a feedback session from a given question number This
     * will not retrieve the list of comments for this question Preconditions: <br>
     * * All parameters are non-null.
     */
    ...
    /**
     * Gets a question+response bundle for questions with responses that is visible
     * to the instructor for a feedback session from a given question number in a
     * given section. This will not retrieve the list of comments for this question
     * Preconditions: <br>
     * * All parameters are non-null.
     */
    public FeedbackSessionResultsBundle getFeedbackSessionResultsForInstructorFromQuestionInSection(
        ...
        return feedbackSessionsLogic.getFeedbackSessionResultsForInstructorFromQuestionInSection(feedbackSessionName,
                courseId, userEmail, questionId, selectedSection);
    }

    /**
     * Gets a question+response bundle for questions with responses that is visible
     * to the instructor for a feedback session. Preconditions: <br>
     * * All parameters are non-null.
     */
    ...
    /**
     * Gets a question+response bundle for questions with responses that is visible
     * to the instructor for a feedback session in a specific section.
     * Preconditions: <br>
     * * All parameters are non-null.
     */
    ...
    public FeedbackResponseAttributes getFeedbackResponse(String feedbackQuestionId, String giverEmail,
            String recipient) {
    ...
    /**
     * Search for FeedbackResponseComment. Preconditions: all parameters are
     * non-null.
     * 
     * @param instructors
     *            a list of InstructorAttributes associated to a googleId, used for
    ...
    /**
     * Update a feedback response comment and return the updated feedback response
     * comment Preconditions: <br>
     * * All parameters are non-null.
     */
     ...
     * 
     * @param key
     *            the GCS blobkey used to fetch the file in Google Cloud Storage
     */
