...
public class FeedbackSessionsLogicTest extends BaseLogicTest {
    ...
    private void testGetFeedbackSessionsListForInstructor() {
        ...
        AssertHelper.assertSameContentIgnoreOrder(finalFsa,
                fsLogic.getFeedbackSessionsListForInstructor(instructorGoogleId, false));
    ...
    private void testCreateAndDeleteFeedbackSession() throws InvalidParametersException, EntityAlreadyExistsException {
        ...
        try {
            ...
            assertEquals("\"test %| test\" is not acceptable to TEAMMATES as a/an feedback session name "
                    ...
                    + "any vertical bar (|) or percent sign (%).", e.getMessage());
        }
    ...
    private void testCopyFeedbackSession() throws Exception {
        ...
        FeedbackSessionAttributes copiedSession = fsLogic.copyFeedbackSession("Copied Session", typicalCourse2.getId(),
        ...
        List<FeedbackQuestionAttributes> questions1 = fqLogic.getFeedbackQuestionsForSession(
                session1InCourse1.getFeedbackSessionName(), session1InCourse1.getCourseId());
        List<FeedbackQuestionAttributes> questions2 = fqLogic
                .getFeedbackQuestionsForSession(copiedSession.getFeedbackSessionName(), copiedSession.getCourseId());
        ...
        try {
            fsLogic.copyFeedbackSession(session1InCourse1.getFeedbackSessionName(), session1InCourse1.getCourseId(),
                    session1InCourse1.getFeedbackSessionName(), session1InCourse1.getCourseId(),
                    instructor2OfCourse1.email);
            signalFailureToDetectException();
    ...
    private void testGetFeedbackSessionDetailsForInstructor() throws Exception {
        ...
        FeedbackSessionStats stats = detailsMap
                .get(newDataBundle.feedbackSessions.get("standard.session").getFeedbackSessionName() + "%"
        ...
        // For private sessions, we mark as completed only when creator has finished all
        // questions.
        assertEquals(0, stats.submittedTotal);
        ...
        // Re-read details
        detailsList = fsLogic
                .getFeedbackSessionDetailsForInstructor(newDataBundle.instructors.get("instructor1OfCourse1").googleId);
        for (FeedbackSessionDetailsBundle details : detailsList) {
            if (details.feedbackSession.getFeedbackSessionName()
                    .equals(newDataBundle.feedbackSessions.get("private.session").getFeedbackSessionName())) {
                stats = details.stats;
                break;
            }
        }
        ...
        detailsList = fsLogic
                .getFeedbackSessionDetailsForInstructor(newDataBundle.instructors.get("instructor2OfCourse1").googleId);
        ...
        stats = detailsMap
                .get(newDataBundle.feedbackSessions.get("private.session.noquestions").getFeedbackSessionName() + "%"
        ...
        detailsList = fsLogic
                .getFeedbackSessionDetailsForInstructor(newDataBundle.instructors.get("instructor1OfCourse3").googleId);
        ...
        AssertHelper.assertContains(expectedSessions, actualSessions);
        stats = detailsMap
                .get(newDataBundle.feedbackSessions.get("private.session.norecipients").getFeedbackSessionName() + "%"
    ...
    private void testGetFeedbackSessionsForCourse() throws Exception {
        ...
        actualSessions = fsLogic.getFeedbackSessionsForUserInCourse("idOfTypicalCourse1",
                "student1InCourse1@gmail.tmt");

        // Student can see sessions 1 and 2. Session 3 has no questions. Session 4 is
        // not yet visible for students.
        String expected = dataBundle.feedbackSessions.get("session1InCourse1").toString() + Const.EOL
        ...
        actualSessions = fsLogic.getFeedbackSessionsForUserInCourse("idOfTypicalCourse2",
                "student1InCourse2@gmail.tmt");
        ...
        expected = dataBundle.feedbackSessions.get("session1InCourse1").toString() + Const.EOL
                + dataBundle.feedbackSessions.get("session2InCourse1").toString() + Const.EOL
        ...
        // We should only have one session here as session 2 is private and this
        // instructor is not the creator.
        actualSessions = fsLogic.getFeedbackSessionsForUserInCourse("idOfTypicalCourse2", "instructor2@course2.tmt");

        assertEquals(actualSessions.get(0).toString(), dataBundle.feedbackSessions.get("session2InCourse2").toString());
        assertEquals(1, actualSessions.size());
        ...
        actualSessions = fsLogic.getFeedbackSessionsForUserInCourse("idOfArchivedCourse",
                "student1InCourse1@gmail.tmt");
    ...
    private void testGetFeedbackSessionQuestionsForStudent() throws Exception {
        ...
        FeedbackSessionQuestionsBundle actual = fsLogic.getFeedbackSessionQuestionsForStudent("First feedback session",
        ...
        // Check that instructors (except the one who is not displayed to student)
        ...
        // Check that student3 get team member's (student4) feedback response as well
        // (for team question).
        actual = fsLogic.getFeedbackSessionQuestionsForStudent("Second feedback session", "idOfTypicalCourse1",
        ...
        try {
            fsLogic.getFeedbackSessionQuestionsForStudent("invalid session", "idOfTypicalCourse1",
                    "student3InCourse1@gmail.tmt");
            signalFailureToDetectException("Did not detect that session does not exist.");
        } catch (EntityDoesNotExistException e) {
            assertEquals(
                    "Trying to get a non-existent feedback session: " + "idOfTypicalCourse1" + "/" + "invalid session",
                    e.getMessage());
        }
        ...
        try {
            fsLogic.getFeedbackSessionQuestionsForStudent("Second feedback session", "idOfTypicalCourse1",
                    "randomUserId");
    ...
    private void testGetFeedbackSessionQuestionsForInstructor() throws Exception {
        ...
        FeedbackSessionQuestionsBundle actual = fsLogic.getFeedbackSessionQuestionsForInstructor(
                "Instructor feedback session", "idOfTypicalCourse2", null, "instructor1@course2.tmt");
        ...
        actual = fsLogic.getFeedbackSessionQuestionsForInstructor("Private feedback session", "idOfTypicalCourse2",
                null, "instructor2@course2.tmt");
        assertEquals(0, actual.questionResponseBundle.size());

        ______TS("private test: is creator");
        actual = fsLogic.getFeedbackSessionQuestionsForInstructor("Private feedback session", "idOfTypicalCourse2",
                null, "instructor1@course2.tmt");
        ...
        try {
            fsLogic.getFeedbackSessionQuestionsForInstructor("invalid session", "idOfTypicalCourse1", null,
    ...
    private void testGetFeedbackSessionResultsForUser() throws Exception {
        ...
        FeedbackSessionResultsBundle results = fsLogic.getFeedbackSessionResultsForStudent(
        ...
        Collections.addAll(expectedStrings, "FSRTest.student1InCourse1@gmail.tmt=student1 In Course1",
                "FSRTest.student2InCourse1@gmail.tmt=student2 In Course1",
                "FSRTest.student4InCourse1@gmail.tmt=student4 In Course1",
                "Team 1.1</td></div>'\"=Team 1.1</td></div>'\"", "Team 1.2=Team 1.2", "Team 1.3=Team 1.3",
                "Team 1.4=Team 1.4", "FSRTest.instr1@course1.tmt=Instructor1 Course1",
                ...
                student2AnonEmail + "=" + student2AnonName, student4AnonEmail + "=" + student4AnonName);
        ...
        Collections.addAll(expectedStrings, "FSRTest.student4InCourse1@gmail.tmt=Team 1.2",
                "FSRTest.student1InCourse1@gmail.tmt=Team 1.1</td></div>'\"",
                "FSRTest.student1InCourse1@gmail.tmt's Team=", "FSRTest.student2InCourse1@gmail.tmt's Team=",
                "FSRTest.student4InCourse1@gmail.tmt's Team=",
                "FSRTest.student2InCourse1@gmail.tmt=Team 1.1</td></div>'\"", "Team 1.1</td></div>'\"=", "Team 1.3=",
                "Team 1.2=", "Team 1.4=", "FSRTest.instr1@course1.tmt=Instructors",
        ...
        Collections.addAll(expectedStrings, getStudentAnonName(responseBundle, "student2InCourse1"),
                getStudentAnonName(responseBundle, "student4InCourse1"),
                "student1 In Course1</td></div>'\" (Team 1.1</td></div>'\")",
                "student2 In Course1 (Team 1.1</td></div>'\")", "student4 In Course1 (Team 1.2)",
                "Instructor1 Course1 (Instructors)", "Team 1.1</td></div>'\"", "Team 1.2", "Team 1.3", "Team 1.4");
        ...
        expectedStrings.clear();
        Collections.addAll(expectedStrings, getResponseId("qn1.resp1", responseBundle) + "={true,true}",
        ...
        results = fsLogic.getFeedbackSessionResultsForInstructor(session.getFeedbackSessionName(),
        ...
        String instructor1AnonEmail = FeedbackSessionResultsBundle.getAnonEmail(FeedbackParticipantType.INSTRUCTORS,
                responseBundle.instructors.get("instructor1OfCourse1").name);
        String instructor1AnonName = FeedbackSessionResultsBundle.getAnonName(FeedbackParticipantType.INSTRUCTORS,
                responseBundle.instructors.get("instructor1OfCourse1").name);
        String instructor2AnonEmail = FeedbackSessionResultsBundle.getAnonEmail(FeedbackParticipantType.INSTRUCTORS,
                responseBundle.instructors.get("instructor2OfCourse1").name);
        String instructor2AnonName = FeedbackSessionResultsBundle.getAnonName(FeedbackParticipantType.INSTRUCTORS,
                responseBundle.instructors.get("instructor2OfCourse1").name);
        Collections.addAll(expectedStrings, "%GENERAL%=%NOBODY%",
                ...
                "FSRTest.instr1@course1.tmt=Instructor1 Course1", "FSRTest.instr2@course1.tmt=Instructor2 Course1",
                student1AnonEmail + "=" + student1AnonName, student2AnonEmail + "=" + student2AnonName,
                student3AnonEmail + "=" + student3AnonName, student6AnonEmail + "=" + student6AnonName,
                instructor1AnonEmail + "=" + instructor1AnonName, instructor2AnonEmail + "=" + instructor2AnonName,
                "Team 1.2=Team 1.2", "Team 1.3=Team 1.3", "Team 1.4=Team 1.4");
        AssertHelper.assertContains(expectedStrings, mapString);
        ...
        mapString = results.emailTeamNameTable.toString();
        expectedStrings.clear();
        Collections.addAll(expectedStrings, "%GENERAL%=", "FSRTest.student1InCourse1@gmail.tmt=Team 1.1</td></div>'\"",
                "FSRTest.student2InCourse1@gmail.tmt=Team 1.1</td></div>'\"",
                "FSRTest.student3InCourse1@gmail.tmt=Team 1.2", "FSRTest.student4InCourse1@gmail.tmt=Team 1.2",
                "FSRTest.student5InCourse1@gmail.tmt=Team 1.3", "FSRTest.student6InCourse1@gmail.tmt=Team 1.4",
                "FSRTest.instr2@course1.tmt=Instructors", "FSRTest.instr1@course1.tmt=Instructors",
                ...
                instructor2AnonEmail + "=" + instructor2AnonName + Const.TEAM_OF_EMAIL_OWNER, "Team 1.3=", "Team 1.2=",
                "Team 1.4=");
        AssertHelper.assertContains(expectedStrings, mapString);
        ...
        mapString = tableToString(results.visibilityTable);
        expectedStrings.clear();
        Collections.addAll(expectedStrings, getResponseId("qn2.resp1", responseBundle) + "={false,false}",
        ...
        results = fsLogic.getFeedbackSessionResultsForInstructorInSection(session.getFeedbackSessionName(),
        ...
        Collections.addAll(expectedStrings, "FSRTest.student1InCourse1@gmail.tmt=student1 In Course1",
                ...
                "FSRTest.student2InCourse1@gmail.tmt=student2 In Course1", "Team 1.4=Team 1.4",
        ...
        mapString = results.emailTeamNameTable.toString();
        expectedStrings.clear();
        Collections.addAll(expectedStrings, "FSRTest.student1InCourse1@gmail.tmt=Team 1.1</td></div>'\"",
                student1AnonEmail + "=" + student1AnonName + Const.TEAM_OF_EMAIL_OWNER,
                student2AnonEmail + "=" + student2AnonName + Const.TEAM_OF_EMAIL_OWNER,
                student3AnonEmail + "=" + student3AnonName + Const.TEAM_OF_EMAIL_OWNER,
                student6AnonEmail + "=" + student6AnonName + Const.TEAM_OF_EMAIL_OWNER,
                instructor1AnonEmail + "=" + instructor1AnonName + Const.TEAM_OF_EMAIL_OWNER,
                "FSRTest.student2InCourse1@gmail.tmt=Team 1.1</td></div>'\"", "Team 1.4=",
        ...
        mapString = tableToString(results.visibilityTable);
        expectedStrings.clear();
        Collections.addAll(expectedStrings, getResponseId("qn3.resp1", responseBundle) + "={true,false}",
                getResponseId("qn4.resp3", responseBundle) + "={true,true}",
                getResponseId("qn2.resp3", responseBundle) + "={false,false}",
        ...
        results = fsLogic.getFeedbackSessionResultsForStudent(session.getFeedbackSessionName(), session.getCourseId(),
                student.email);
        ...
        mapString = results.emailTeamNameTable.toString();
        expectedStrings.clear();
        Collections.addAll(expectedStrings, "FSRTest.student1InCourse1@gmail.tmt=Team 1.1</td></div>'\"", "Team 1.2=",
                FeedbackSessionResultsBundle.getAnonEmail(FeedbackParticipantType.TEAMS,
                        responseBundle.students.get("student3InCourse1").team),
                "FSRTest.instr1@course1.tmt=Instructors");
        AssertHelper.assertContains(expectedStrings, mapString);
        ...
        // it is a private session. (to protect anonymity during session type
        // conversion)"
        mapString = tableToString(results.visibilityTable);
        expectedStrings.clear();
        Collections.addAll(expectedStrings, getResponseId("p.qn1.resp1", responseBundle) + "={true,true}",
        ...
        try {
            ...
            assertEquals("Trying to view a non-existent feedback session: " + session.getCourseId() + "/"
    ...
    private void testGetFeedbackSessionResultsSummaryAsCsv() throws Exception {
        ...
        String export = fsLogic.getFeedbackSessionResultsSummaryAsCsv(session.getFeedbackSessionName(),
                session.getCourseId(), instructor.email, null, true, true);

        String[] expected = {
                // CHECKSTYLE.OFF:LineLength csv lines can exceed character limit
                "Course,\"" + session.getCourseId() + "\"", "Session Name,\"" + session.getFeedbackSessionName() + "\"",
        ...
        export = fsLogic.getFeedbackSessionResultsSummaryAsCsv(session.getFeedbackSessionName(), session.getCourseId(),
                instructor.email, null, false, true);
        ...
        final String questionId = fqLogic
                .getFeedbackQuestion(session.getFeedbackSessionName(), session.getCourseId(), questionNum).getId();

        export = fsLogic.getFeedbackSessionResultsSummaryAsCsv(session.getFeedbackSessionName(), session.getCourseId(),
                instructor.email, questionId, true, true);
        ...
        expected = new String[] {
                // CHECKSTYLE.OFF:LineLength csv lines can exceed character limit
                "Course,\"" + session.getCourseId() + "\"", "Session Name,\"" + session.getFeedbackSessionName() + "\"",
                "", "", "Question 1,\"What do you like best about our product?\"", "", "Summary Statistics,",
                "Choice, Response Count, Percentage", "\"It's good\",1,50", "\"It's perfect\",1,50", "",
                "Team,Giver's Full Name,Giver's Last Name,Giver's Email,Recipient's Team,Recipient's Full Name,Recipient's Last Name,Recipient's Email,Feedback",
                ...
                "\"Team 1.1</td></div>'\"\"\",\"student4 In Course1\",\"Course1\",\"student4InCourse1@gmail.tmt\",\"Team 1.1</td></div>'\"\"\",\"student4 In Course1\",\"Course1\",\"student4InCourse1@gmail.tmt\",\"No Response\"",
                "\"Team 1.2\",\"student5 In Course1\",\"Course1\",\"student5InCourse1@gmail.tmt\",\"Team 1.2\",\"student5 In Course1\",\"Course1\",\"student5InCourse1@gmail.tmt\",\"No Response\"",
                "", "", "Question 2,\"What do you like best about the class' product?\"", "", "Summary Statistics,",
                "Choice, Response Count, Percentage", "\"It's good\",1,50", "\"It's perfect\",1,50", "",
                "Team,Giver's Full Name,Giver's Last Name,Giver's Email,Recipient's Team,Recipient's Full Name,Recipient's Last Name,Recipient's Email,Feedback",
                ...
                "", "", "Question 3,\"What can be improved for this class?\"", "", "Summary Statistics,",
                "Choice, Response Count, Percentage", "\"Content\",0,0", "\"Teaching style\",0,0", "\"Other\",1,100",
                "",
                "Team,Giver's Full Name,Giver's Last Name,Giver's Email,Recipient's Team,Recipient's Full Name,Recipient's Last Name,Recipient's Email,Feedback",
        ...
        expected = new String[] {
                ...
                "Course,\"" + session.getCourseId() + "\"", "Session Name,\"" + session.getFeedbackSessionName() + "\"",
                "", "", "Question 1,\"What do you like best about our product?\"", "", "Summary Statistics,",
                "Choice, Response Count, Percentage", "\"It's good\",2,66.67", "\"It's perfect\",1,33.33", "", "",
                ...
                "\"Team 1.2\",\"student5 In Course1\",\"Course1\",\"student5InCourse1@gmail.tmt\",\"Team 1.2\",\"student5 In Course1\",\"Course1\",\"student5InCourse1@gmail.tmt\",\"No Response\"",
                "", "", "Question 2,\"What do you like best about the class' product?\"", "", "Summary Statistics,",
                "Choice, Response Count, Percentage", "\"It's good\",1,33.33", "\"It's perfect\",2,66.67", "", "",
                "Team,Giver's Full Name,Giver's Last Name,Giver's Email,Recipient's Team,Recipient's Full Name,Recipient's Last Name,Recipient's Email,Feedbacks:,\"It's good\",\"It's perfect\"",
                ...
                "", "", "Question 3,\"Choose all the food you like\"", "", "Summary Statistics,",
                "Choice, Response Count, Percentage", "\"Pizza\",1,16.67", "\"Pasta\",2,33.33",
                "\"Chicken rice\",1,16.67", "\"Other\",2,33.33", "", "",
        ...
        expected = new String[] {
                // CHECKSTYLE.OFF:LineLength csv lines can exceed character limit
                "Course,\"" + session.getCourseId() + "\"", "Session Name,\"" + session.getFeedbackSessionName() + "\"",
                "", "", "Question 1,\"Rate our product.\"", "", "Summary Statistics,",
                ...
                "\"Team 1.1</td></div>'\"\"\",\"student4 In Course1\",\"Course1\",\"student4InCourse1@gmail.tmt\",\"Team 1.1</td></div>'\"\"\",\"student4 In Course1\",\"Course1\",\"student4InCourse1@gmail.tmt\",\"No Response\"",
                "\"Team 1.2\",\"student5 In Course1\",\"Course1\",\"student5InCourse1@gmail.tmt\",\"Team 1.2\",\"student5 In Course1\",\"Course1\",\"student5InCourse1@gmail.tmt\",\"No Response\"",
                "", "", "Question 2,\"Rate our product.\"", "", "Summary Statistics,",
                "Team, Recipient, Average, Minimum, Maximum", "\"Instructors\",\"Instructor2 Course1\",1,1,1",
        ...
        expected = new String[] {
                ...
                "Summary Statistics,", "Option, Average Points, Total Points, Received Points",
                "\"Fun\",50.5,101,81,20", "\"Grades\",49.5,99,19,80", "", "",
                ...
                "\"Team 1.1</td></div>'\"\"\",\"student4 In Course1\",\"Course1\",\"student4InCourse1@gmail.tmt\",\"Team 1.1</td></div>'\"\"\",\"student4 In Course1\",\"Course1\",\"student4InCourse1@gmail.tmt\",\"No Response\"",
                "\"Team 1.2\",\"student5 In Course1\",\"Course1\",\"student5InCourse1@gmail.tmt\",\"Team 1.2\",\"student5 In Course1\",\"Course1\",\"student5InCourse1@gmail.tmt\",\"No Response\"",
                "", "", "Question 2,\"Split points among the teams\"", "", "Summary Statistics,",
                "Team, Recipient, Average Points, Total Points, Received Points",
                "\"\",\"Team 1.1</td></div>'\"\"\",80,80,80", "\"\",\"Team 1.2\",20,20,20", "", "",
                "Team,Giver's Full Name,Giver's Last Name,Giver's Email,Recipient's Team,Recipient's Full Name,Recipient's Last Name,Recipient's Email,Feedback",
                ...
                "", "", "Question 3,\"How much has each student worked?\"", "", "Summary Statistics,",
                "Team, Recipient, Average Points, Total Points, Received Points",
        ...
        expected = new String[] {
                // CHECKSTYLE.OFF:LineLength csv lines can exceed character limit
                "Course,\"FSQTT.idOfTypicalCourse1\"", "Session Name,\"CONSTSUM Session\"", "", "",
                "Question 1,\"How important are the following factors to you? Give points accordingly.\"", "",
                "Team,Giver's Full Name,Giver's Last Name,Giver's Email,Recipient's Team,Recipient's Full Name,Recipient's Last Name,Recipient's Email,Feedbacks:,\"Grades\",\"Fun\"",
        ...
        expected = new String[] {
                ...
                "Perceived Contribution (PC) = the average value of student's contribution as perceived by the team members.",
                "Team, Name, Email, CC, PC, Ratings Recieved",
                "\"" + student1AnonName + "'s Team\",\"" + student1AnonName
                        + "\",\"-\",\"100\",\"N/A\",\"N/A, N/A, N/A\"",
                "\"" + student2AnonName + "'s Team\",\"" + student2AnonName
                        + "\",\"-\",\"Not Submitted\",\"N/A\",\"N/A, N/A, N/A\"",
                "\"" + student3AnonName + "'s Team\",\"" + student3AnonName
                        + "\",\"-\",\"Not Submitted\",\"N/A\",\"N/A, N/A, N/A\"",
                "\"" + student4AnonName + "'s Team\",\"" + student4AnonName
                        + "\",\"-\",\"Not Submitted\",\"N/A\",\"N/A, N/A, N/A\"",
                "\"" + student5AnonName + "'s Team\",\"" + student5AnonName
                        + "\",\"-\",\"Not Submitted\",\"N/A\",\"N/A\"",
                "", "",
                "Team,Giver's Full Name,Giver's Last Name,Giver's Email,Recipient's Team,Recipient's Full Name,Recipient's Last Name,Recipient's Email,Feedback",
                "\"Team 1.1</td></div>'\"\"\",\"student1 In Course1</td></div>'\"\"\",\"Course1</td></div>'\"\"\",\"student1InCourse1@gmail.tmt\",\""
                        + student1AnonName + "'s Team\",\"" + student1AnonName + "\",\"Unknown user\",\"-\",\"\"",
                "\"Team 1.1</td></div>'\"\"\",\"student1 In Course1</td></div>'\"\"\",\"Course1</td></div>'\"\"\",\"student1InCourse1@gmail.tmt\",\"Team 1.1</td></div>'\"\"\",\"student1 In Course1</td></div>'\"\"\",\"Course1</td></div>'\"\"\",\"student1InCourse1@gmail.tmt\",\"No Response\"",
        ...
        expected = new String[] {
                ...
                "Summary Statistics,", ",\"Yes (Weight: 1.25)\",\"No (Weight: -1.7)\",Average",
                ...
                "", "Summary Statistics,", ",\"Yes (Weight: 1.25)\",\"No (Weight: 1)\",Average",
        ...
        expected = new String[] {
                // CHECKSTYLE.OFF:LineLength csv lines can exceed character limit
                "Course,\"" + session.getCourseId() + "\"", "Session Name,\"" + session.getFeedbackSessionName() + "\"",
                "", "", "Question 1,\"Rank the other students.\"", "", "Summary Statistics,",
                ...
                "Summary Statistics,", "Option, Average Rank, Ranks Received",
                "\"Quality of progress reports\",2.5,2,3", "\"Time management\",2,3,2,1,2",
                "\"Quality of work\",2.4,1,4,3,3,1", "\"Teamwork and communication\",1.75,4,1,1,1", "", "",
        ...
        export = fsLogic.getFeedbackSessionResultsSummaryAsCsv(session.getFeedbackSessionName(), session.getCourseId(),
                instructor.email, null, true, false);
    ...
    private void testUpdateFeedbackSession() throws Exception {
        ...
        FeedbackSessionAttributes fsa = FeedbackSessionAttributes
                .builder("asdf_randomName1423", "idOfTypicalCourse1", "").build();

        try {
            fsLogic.updateFeedbackSession(fsa);
            signalFailureToDetectException();
        } catch (EntityDoesNotExistException edne) {
            assertEquals("Trying to update a non-existent feedback session: " + fsa.getCourseId() + "/"
                    + fsa.getFeedbackSessionName(), edne.getMessage());
        }
        ...
        assertEquals(fsa.toString(),
                fsLogic.getFeedbackSession(fsa.getFeedbackSessionName(), fsa.getCourseId()).toString());
    }
    ...
    private void testPublishUnpublishFeedbackSession() throws Exception {
        ...
        FeedbackSessionAttributes sessionPublished = fsLogic
                .getFeedbackSession(sessionUnderTest.getFeedbackSessionName(), sessionUnderTest.getCourseId());
        ...
        assertEquals(sessionUnderTest.toString(),
        ...
        try {
            fsLogic.publishFeedbackSession(sessionUnderTest);
            signalFailureToDetectException(
                    "Did not catch exception signalling that private session can't " + "be published.");
        } catch (InvalidParametersException e) {
            assertEquals("Error publishing feedback session: Session is private and can't be published.",
                    e.getMessage());
        }
        ...
        try {
            fsLogic.unpublishFeedbackSession(sessionUnderTest);
            signalFailureToDetectException(
                    "Did not catch exception signalling that private session should " + "not be published");
        } catch (InvalidParametersException e) {
    ...
    private FeedbackSessionAttributes getNewFeedbackSession() {
        return FeedbackSessionAttributes.builder("fsTest1", "testCourse", "valid@email.tmt")
                .withFeedbackSessionType(FeedbackSessionType.STANDARD).withCreatedTime(new Date())
                .withStartTime(new Date()).withEndTime(new Date()).withSessionVisibleFromTime(new Date())
                .withResultsVisibleFromTime(new Date()).withGracePeriod(5).withSentOpenEmail(true)
                .withSentPublishedEmail(true).withInstructions(new Text("Give feedback.")).build();
    }
    ...
    private FeedbackQuestionAttributes getQuestionFromDatastore(String jsonId) {
        ...
        questionToGet = fqLogic.getFeedbackQuestion(questionToGet.feedbackSessionName, questionToGet.courseId,
